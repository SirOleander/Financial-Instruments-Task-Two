"""sec.py — SEC EDGAR: network client + fundamentals extraction.

Merges the old `C_client` (HTTP + companyfacts/iXBRL parsing, `select_best_fact` with the
annual-duration floor fix) and `D_pipeline` (per-ticker orchestration, calculated positions,
YTD de-cumulation). They were always one concern split across a file boundary: D_pipeline
used nine of C_client's functions and nothing else did.

NAME CLASH RESOLVED ON MERGE. Both files defined `_infer_fiscal_year_end_month`, with
DIFFERENT signatures and different inputs:
  * C_client's read `reportDate` from the submissions frame   -> `_fye_month_from_filings`
  * D_pipeline's read `form`/`fact_end_date` from extracted rows -> `_fye_month_from_facts`
Concatenating them unchanged would have let the second shadow the first and silently feed
...
MAX_RETRIES = 3
COMPANYFACTS_URL_TEMPLATE = config.SEC_DATA_BASE_URL + "/api/xbrl/companyfacts/CIK{cik_10}.json"
...
def get_position_statement_map(ticker: str) -> dict[str, str]:
    return {
        ...
        for statement_type, positions in config.get_financial_positions_for_ticker(ticker).items()
...
def _fye_month_from_filings(annual_filings: pd.DataFrame) -> int:
...
def select_target_accessions(filings: pd.DataFrame) -> pd.DataFrame:
    ...
    target = filings[
        filings["form"].isin(config.TARGET_FORM_TYPES)
    ...
    fiscal_year_end_month = _fye_month_from_filings(
...
def fetch_companyfacts(session: requests.Session, ticker: str, cik: str) -> dict[str, Any]:
    ...
    url = COMPANYFACTS_URL_TEMPLATE.format(cik_10=config.cik_10(cik))
...
def extract_candidate_rows_for_ticker(
    ...
    for position in get_positions_for_ticker(ticker):
        for concept_priority, concept in enumerate(
            config.get_concepts_for_financial_position(ticker, position),
            start=1,
        ):
            for fact in facts_for_concept(companyfacts, concept):
                if fact.get("accn") not in accession_set:
                    continue
                if fact.get("form") not in config.TARGET_FORM_TYPES:
                    continue
...
def _positions_needed_for_ticker(ticker: str) -> tuple[list[str], dict[str, str | None]]:
    ...
    for inline_position, rule in config.get_inline_financial_items_for_ticker(ticker).items():
        position_statement.setdefault(inline_position, rule.get("statement_type"))

    for rule in config.get_calculated_financial_items_for_ticker(ticker).values():
...
def build_standardized_rows(
    ...
    for filing in selected_filings.to_dict("records"):
        ...
        for position in positions:
            ...
            row = {
                **company_metadata,
                "cik": filing.get("filing_cik") or config.get_cik(ticker),
                "form": form,
            ...
            if selected is None:
                ...
                if config.FINANCIAL_POSITION_SIGN_RULES.get(position) == "store_positive_absolute_value" and pd.notna(value):
                    value = abs(value)
...
def get_target_tickers() -> list[str]:
    unknown_groups = sorted(set(TARGET_GROUPS) - set(config.COMPANY_GROUPS))
    if unknown_groups:
    ...
    return sorted(
        {
            ...
            for ticker in config.COMPANY_GROUPS[group]
            if ticker in config.ACTIVE_TICKERS and config.get_sector(ticker) in config.ACTIVE_SECTORS
        }
    )
...
def process_ticker(session: requests.Session, ticker: str) -> int:
    ...
    for cik in ciks:
        ...
        filings["filing_cik"] = config.cik_10(cik)
    ...
    for cik in ciks:
        ...
        companyfacts_rows = extract_candidate_rows_for_ticker(
            ticker=ticker,
        ...
        ixbrl_rows = extract_inline_candidate_rows_for_ticker(
            session=session,
            ticker=ticker,
            cik=cik_10,
            selected_filings=selected_for_cik,
    ...
    standardized = apply_calculated_financial_items(
        ticker=ticker,
...
def _fye_month_from_facts(df: pd.DataFrame) -> int:
...
def decumulate_ytd_flows(rows: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    df = rows.copy()
    fiscal_year_end_month = _fye_month_from_facts(df)
