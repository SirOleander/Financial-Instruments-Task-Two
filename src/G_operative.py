"""
G_operative.py — qualitative "operative score" via LLM, STAGE 1 (US EDGAR only).

Greenfield. For each US 10-K / 10-Q report, an LLM reads the filing's narrative
sections (MD&A / Risk Factors / Business) and returns ONE integer 1-5 judging the
forward-looking qualitative tone / competitive outlook of THAT disclosure (5=clearly
positive, 3=neutral/mixed, 1=clearly negative). No KPIs/numbers are fed. The 1-5 is
rescaled to [0,1] as (raw-1)/4 = strategic_score. Results go in a NEW table
`operative_scores`. Purely additive — never touches financial_facts / daily_prices /
target_63d / kpi_values / scores.

LOOK-AHEAD LOCK: the prompt forbids the model from using ANY outside/after-the-fact
knowledge of the company; it scores ONLY the provided text as of the filing date.

API: LiteLLM (OpenAI-compatible) at BASE_URL, model qwen3.5-122b-a10b, temperature 0.
Key read ONLY from env var LITELLM_API_KEY (never hardcoded/printed/written). Caching:
skip the API call if the accession is already scored. Model + prompt_version stored per row.

USAGE (run from inside src/, requires env var LITELLM_API_KEY for live calls):
    python G_operative.py --dry-run     # 1a: extract + prompt + (live call if key) on ~5 filings, NO writes
    python G_operative.py --write       # 1b: full US run, cache by accession, write + verify
"""
...
def recover_mdna_ex13(session, rep: dict) -> tuple[str, str | None]:
    ...
    try:
        docs = C_client.list_filing_inline_documents(
    ...
    for name in [d for d in docs if is_ex13(d)]:
        try:
            html = C_client.fetch_filing_document(
...
def resolve_ciks(session) -> dict[str, str]:
    ...
    j = C_client.request_json(session, SEC_COMPANY_TICKERS_URL)
...
def load_intl_20f_reports(session=None, since_year: int | None = None) -> list[dict]:
    ...
    session = session or C_client.make_session()
    ciks = resolve_ciks(session)
    reports: list[dict] = []
    for db_ticker, cik in ciks.items():
        subs = C_client.fetch_submissions(session, db_ticker, cik)
